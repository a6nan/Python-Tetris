...
class Block(pg.sprite.Sprite):
    def __init__(self, tetromino, pos, is_special=False):
        ...
        if self.is_special:
            self.image.fill((255, 50, 50, 100), special_flags=pg.BLEND_RGBA_ADD)
    ...
    def set_rect_pos(self):
        if self.tetromino.current == True:
            pos = self.pos
        elif self.tetromino.current == False:
            pos = self.next_pos
        else:
            pos = self.hold_pos
...
class Tetromino:
    def __init__(self, tetris, current=True, shape=None, image=None, is_special=None):
        self.tetris = tetris
        self.shape = shape if shape else random.choice(list(TETROMINOES.keys()))
        self.image = image if image else random.choice(tetris.app.images)

        if is_special is None:
            self.is_special = True if random.random() < 0.01 else False
        else:
            self.is_special = is_special

        self.blocks = [Block(self, pos, is_special=self.is_special) for pos in TETROMINOES[self.shape]]
